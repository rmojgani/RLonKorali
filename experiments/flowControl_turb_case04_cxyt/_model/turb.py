    ...
    def step( self, action=None ):
        ...
        if (action is not None):
           ...
           # Initlize action
            ...
            #    self.myplot2(arr_action, forcing, 'nu')
            #print(y2.shape)
            #print(forcing.shape)
            #print(x2)
            #stop_forcing
...
#        else:
    ...
    def reward(self):
        ...
        #myreward = - np.linalg.norm( (enstrophy[0:kmax] - enstrophy_ref)/ enstrophy_ref )
        ...
        if (v0 is None):
            if (u0 is None):
                ...
                if self.RL:
                    ...
                    # Gaussian noise (accordingnActiongrid to https://arxiv.org/pdf/1906.07672.pdf)
                    np.random.seed( SEED )
...
#        print( np.stack((Kplot[0:kmax,0], energy[0:kmax]),axis=0).T.shape   )
    ...
    def myplot2(self, contour1, contour2, append_str=''):
        stepnum = self.stepnum
        NX = self.NX
        VMIN, VMAX = -0.075, 0.075
        # --------------
        plt.figure(figsize=(8,14/3))
        fig, axs = plt.subplots(1,2)

        ax = axs[0]
        ax.pcolor(contour1, vmin=VMIN, vmax=VMAX)
        ax = axs[1]
        pcm = ax.pcolor(contour2, vmin=VMIN, vmax=VMAX)
        
        fig.colorbar(pcm, ax=axs[:], shrink=0.95, location='top')
        #for icount in [1,2]:
        #    plt.subplot(3,2,icount)
        #    plt.colorbar()
        
        filename = '2Dturb_N'+str(NX)+'_'+str(stepnum)+'_v2_'+append_str
        plt.savefig(filename+'.png', bbox_inches='tight', dpi=450)
