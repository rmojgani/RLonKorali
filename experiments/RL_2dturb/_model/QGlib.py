...
def QGfun(i, kk, ll, D_Dx, D_Dy, Grad, Lapl, Grad8,
    ...
    #print(i, '->',  np.linalg.norm(psi_1[1]), np.linalg.norm(D_Dx), tau_f, tau_d, nu )
    #NL terms =
    #J(psi, qc+R) 
...
def qtp(q1_s, q2_s, ll, kk):
    """Invert PV"""
    kkll = (ll[:, np.newaxis] ** 2 + kk[np.newaxis, :] ** 2)
    
    psi_bt = -(q1_s + q2_s)
    psi_bt[1:,1:] = psi_bt[1:,1:]/kkll[1:,1:]/2.  # (psi_1 + psi_2)/2
    psi_bt[1:,0] = psi_bt[1:,0]/kkll[1:,0]/ 2.  # (psi_1 + psi_2)/2
    psi_bt[0,1:] = psi_bt[0,1:]/kkll[0,1:]/ 2.  # (psi_1 + psi_2)/2
    psi_bt[0, 0] = 0.
    
    psi_bc = -(q1_s - q2_s) / (kkll + 2. ) / 2.  # (psi_1 - psi_2)/2
